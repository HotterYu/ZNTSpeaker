    		...
    		try
    		{
				...
				if(result == 0)
				{
					...
					for(int i=0;i<len;i++)
					{
						...
						//DBManager.INSTANCE.insertSong(infor);
					}
					/*String time = getInforFromJason(jsonObject, RESULT_INFO);
					httpResult.setResult(true, time);
					int songCount = DBManager.INSTANCE.getSongCount();
					List<SongInfor> tempList = DBManager.INSTANCE.getSongList(0, 0);*/
					if(tempList.size() > 0)
					{
						for(int i=0;i<tempList.size();i++)
						{
							DBManager.INSTANCE.insertSong(tempList.get(i));
						}
					}
