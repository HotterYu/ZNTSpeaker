...
public class MyExceptionHandler  implements UncaughtExceptionHandler
{
	...
	private void senCrashBraodcast()
	{
		Intent intent = new Intent();  
		intent.setAction("com.znt.dianyin.APP_CRASH");  
		
		try {
			
			String curSign = ApkTools.getSignature(mContext);
			int versionCode = SystemUtils.getPkgInfo(mContext).versionCode;
			Bundle bundle = new Bundle();
			bundle.putInt("VERSION_CODE", versionCode);
			bundle.putString("VERSION_SIGN", curSign);
	...
	private boolean handleException(Throwable ex) {
		...
		//浣跨敤Toast鏉ユ樉绀哄紓甯镐俊鎭�
				new Thread() {
					...
					public void run() {
						Looper.prepare();
						Toast.makeText(mContext, "程序异常，请重试", 0).show();
						Looper.loop();
					}
				}.start();
	...
	private String saveCrashInfo2File(Throwable ex) 
	{
		...
		try 
		{
			...
			fos.close();
			emailManager.sendEmail("盒子异常_id_" + devId + "_" + DateUtils.getStringDate(), sb.toString());
			return fileName;
		} 
		catch (Exception e) 
		{
