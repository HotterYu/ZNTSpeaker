...
public class LocalDataEntity 
{
	...
	private final String LAST_SERVER_TIME = "LAST_SERVER_TIME";
	private final String LAST_REBOOT_TIME = "LAST_REBOOT_TIME";
	private final String REBOOT_COUNT = "REBOOT_COUNT";
	...
	public DeviceInfor getDeviceInfor()
	{
		...
		if(TextUtils.isEmpty(code))
			name = sharedPre.getData(DEVICE_NAME, "DianYin_" + b.MODEL);
		else
			name = sharedPre.getData(DEVICE_NAME, "DianYin_" + code);
	...
	{
		if(TextUtils.isEmpty(PushModelConstant.DEVICE_MAC))
		{
			String macAddr = Secure.getString(context.getContentResolver(), Secure.ANDROID_ID);;
			PushModelConstant.DEVICE_MAC = macAddr +  PushModelConstant.UUID_TAG;
			return PushModelConstant.DEVICE_MAC;//sharedPre.getData(DEVICE_ID, NetWorkUtils.getMacAddress() + PushConstant.UUID_TAG);
		}
		else
			return PushModelConstant.DEVICE_MAC;
	}
